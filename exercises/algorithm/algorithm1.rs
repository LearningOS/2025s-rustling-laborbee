...
impl<T> LinkedList<T> {
        ...
        T: PartialOrd,
    {
        ...
        while let Some(current) = a_current {
            let val = unsafe { &(*current.as_ptr()).val };
            merged_list.add(unsafe { std::ptr::read(val) });
            a_current = unsafe { (*current.as_ptr()).next };
        }

        while let Some(current) = b_current {
            let val = unsafe { &(*current.as_ptr()).val };
            merged_list.add(unsafe { std::ptr::read(val) });
            b_current = unsafe { (*current.as_ptr()).next };
        }
